> **Archived.** This is the Raspberry-Pi-over-the-network setup, which was the
> primary route until the project moved to a Jetson Orin Nano driving the arm
> directly over UART. It is kept because the code still supports it in full —
> `connection:=tcp` and `source:=mjpeg` are unchanged and remain the launch
> defaults — and because it is the fallback if the UART link gives trouble.
>
> For the current setup see [../README.md](../README.md).
>
> Note that some tuning figures below are stale; the values in
> [../CLAUDE.md](../CLAUDE.md) and the node sources are authoritative.
