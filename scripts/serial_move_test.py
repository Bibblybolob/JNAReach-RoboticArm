...
def check_link(mc, samples, tolerance):
    ...
    if not good:
        ...
        print('cleared the adapter.\n')
        print('Do NOT keep guessing from here. -1 is what pymycobot returns')
        print('for "no valid angles", which is the same answer whether')
        print('nothing came back at all, or bytes came back at the wrong')
        print('rate. Those need opposite fixes. Get the raw bytes:\n')
        print('    ./scripts/probe_uart_bridge.py poke\n')
        print('Silence at every rate means nothing is transmitting toward')
        print('you -- swap the two signal wires, and check RESET really is')
        print('strapped to GND. Bytes but no frames means the wiring is')
        print('sound and it is a rate or signal problem.\n')
        print('(`listen` is no help when the Pi is out: with nothing else on')
        print('the bus there is no traffic to overhear. It only works while')
        print('some other master is polling the arm.)')
        return False, None
